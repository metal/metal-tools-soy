...
export default function validateInternal(
	_: SoyContext,
	jsContext: JSContext
): Result {
	...
	const missingInternal = params
		.filter(node => {
			...
			return (
				isInternalName(name) && !JSContext.hasAttribute(node.value, 'internal')
			);
		})
	...
	return toResult(
		false,
		`Based on their name, these attributes should have the ${chalk.yellow(
			'.internal()'

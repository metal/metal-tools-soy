...
export async function main(argv: Array<string>): Promise<void> {
	const cli = program
		...
		.option(
			'-i, --ignore <ignore>',
			'A glob to ignore files, if passed a directory'
		)
	...
	try {
		...
		console.log(
			chalk.red(`Failed to find Soy files using path: "${filePath}"`)
		);
	...
	const validations = await Promise.all(
		files.map(file => validate(file, config))
	);
	const failed = validations.filter(([_, result]) => !result.status);
	const passed = validations.filter(([_, result]) => result.status);

	console.log(
		chalk[failed.length ? 'red' : 'green'](
			`${failed.length} out of ${validations.length} file(s) have problems:\n`
		)
	);

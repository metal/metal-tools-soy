...
export default class JSContext {
	...
	getClassMethodNames(): Array<string> {
		...
		if (
			this._defaultBinding &&
			T.isClassDeclaration(this._defaultBinding.path.node)
		) {
	...
	getParentClassName(): string | null {
		if (
			...
			return JSContext.getNameOrMemberName(
				this._defaultBinding.path.node.superClass
			);
		}
		...
		if (this._defaultBinding) {
			for (let i = 0; i < this._defaultBinding.referencePaths.length; i++) {
				...
				if (
					...
					params = <Array<T.ObjectProperty>>(
						parentPath.parentPath.node.right.properties.filter(node =>
	...
	getParamNames(): Array<string> {
		return this.getParams().map(param => JSContext.getKeyName(param.key));
	}
			...
			ExportDefaultDeclaration(path) {
				...
				if (T.isIdentifier(path.node.declaration)) {
					binding = path
						...
						.scope.getBinding(path.node.declaration.name);
				}
			}
		});

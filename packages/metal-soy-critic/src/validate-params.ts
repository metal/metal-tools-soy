...
function getImplicitParams(
	...
	return Object.keys(paramsMap)
		...
		.reduce(
			(acc, value) => acc.concat(paramsMap[value]),
			initialImplicitParams
		);
}
...
export default function validateParams(
	...
	const implicitParams = className
		? getImplicitParams(className, config.implicitParams)
		: [];

	const missingParams = soyContext
		.getRenderParams()
		.map(param => param.name)
		.filter(
			param =>
				!jsParams.includes(param) &&
				!classMethods.includes(param) &&
				!implicitParams.includes(param)
		);

	if (missingParams.length) {
		return toResult(
			false,
			`These params can't be found in ${chalk.yellow(
				'STATE'
			)} or your ${chalk.yellow('Class')}:\n\n` + joinErrors(missingParams)
		);
	}

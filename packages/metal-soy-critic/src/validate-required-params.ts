...
export default function validateRequiredParams(
	...
	const requiredJSParams = new Set<string>(
		jsParams
			.filter(
				node =>
	...
	const requiredSoyParams = new Set<string>(
		soyParams.filter(param => param.required).map(({ name }) => name)
	);
	...
	if (missingInJS.size) {
		messages.push(
			`These attributes are ${chalk.yellow(
				'required'
			)} in your Soy Template but not in your Component:\n\n` +
	...
	if (missingInSoy.size) {
		messages.push(
			`These attributes are ${chalk.yellow(
				'.required()'
			)} in your Component but not in your Template:\n\n` +

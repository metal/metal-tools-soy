...
function evaluateFile(input: string, output: string) {
	...
	const generator = generateSourceMapFile(
		input,
		sourceContent,
		generatedContent,
		sourceName
	...
	saveFile(
		evaluateGeneratedFile(generatedContent, fileName),
		path.resolve(path.dirname(input), `${input}.js`)
	);
...
function evaluateFilePath(input: string): boolean {
	...
	if (!existFile(input)) {
		throw logger.error(
			`Could not resolve ${sourceName} file in path ${output}`
		);
	}

	if (!existFile(`${input}.js`)) {
		throw logger.error(
			'It is not possible to generate the source map without the generated file.',
			`${input}.js`
		);
	}

...
export default function(
	...
	const partialMapping: PartialMapping[] = createPartialMapping(
		astSoy,
		sourceName
	);
	const mapping: Mapping[] = crossMapping(
		astJs,
		sortPartialMapping(partialMapping)
	);
...
function crossMapping(
	...
	return runEvaluators(traverseJs, ast, soyPartialMapping, evaluatorsJs);
}
...
function createPartialMapping(
	...
	return runEvaluators(traverseSoy, ast, source, evaluatorsSoy);
}
...
function runEvaluators<T, O, U, P>(
	...
	evaluatorsKeys.forEach(name => {
		(<Visitor>visitor)[name.toString()] = (node: File): void => {
			...
			const res: P[] = evaluator(node, arg, ast);

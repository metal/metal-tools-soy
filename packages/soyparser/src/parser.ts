...
const namespace: P.Parser<Array<string>> = P.lazy(() =>
	P.alt(
...
const numberLiteral = nodeMap(
	S.NumberLiteral,
	P.seq(P.oneOf('+-').fallback(''), joined(P.digit, P.string('.'))).map(
		([sign, number]) => parseFloat(sign + number)
	)
);
...
const param = P.lazy(() =>
	nodeMap(
		S.Param,
		P.string('{param').then(spaced(identifierName)),
		P.alt(
...
const functionCall = P.lazy(() =>
	nodeMap(S.FunctionCall, identifierName, lparen.then(functionArgs))
);

const functionArgs: P.Parser<Array<S.Expression>> = P.lazy(() =>
	P.alt(
		rparen.result([]),
...
const mapLiteral = nodeMap(
	S.MapLiteral,
	lbracket.then(P.alt(spaced(mapItem).many(), rbracket.result([])))
);
...
const call = nodeMap(
	...
	P.alt(
		...
		rbrace.then(spaced(param).many()).skip(spaced(closeCmd('call')))
	)
);
...
const delCall = nodeMap(
	...
	P.alt(
		spaced(closingBrace).result([]),
		rbrace.then(spaced(param).many()).skip(spaced(closeCmd('delcall')))
	)
);
...
const paramDeclaration = nodeMap(
	...
	spaced(colon).then(withAny(rbrace))
);
...
const delTemplate = nodeMap(
	...
	P.string('{deltemplate')
		.skip(P.whitespace)
		.then(templateName),
	optional(P.seq(P.whitespace, P.string('variant=')).then(interpolation('"'))),
...
function nodeMap<T, U>(
	mapper: (mark: S.Mark, a1: T) => U,
	p1: P.Parser<T>
): P.Parser<U>;
function nodeMap<T, U, V>(
	mapper: (mark: S.Mark, a1: T, a2: U) => V,
	p1: P.Parser<T>,
	p2: P.Parser<U>
): P.Parser<V>;
function nodeMap<T, U, V, W>(
	mapper: (mark: S.Mark, a1: T, a2: U, a3: V) => W,
	p1: P.Parser<T>,
	p2: P.Parser<U>,
	p3: P.Parser<V>
): P.Parser<W>;
function nodeMap<T, U, V, W, X>(
	mapper: (mark: S.Mark, a1: T, a2: U, a3: V, a4: W) => X,
	p1: P.Parser<T>,
	p2: P.Parser<U>,
	p3: P.Parser<V>,
	p4: P.Parser<W>
): P.Parser<X>;
function nodeMap<T, U, V, W, X, Y>(
	mapper: (mark: S.Mark, a1: T, a2: U, a3: V, a4: W, a5: X) => Y,
	p1: P.Parser<T>,
	p2: P.Parser<U>,
	p3: P.Parser<V>,
	p4: P.Parser<W>,
...
): P.Parser<S.Expression> {
	const spacedEnd = P.optWhitespace.then(end);

	return realExpression(spacedEnd, stack).or(otherExpression(spacedEnd, stack));
}

function realExpression<T>(
	end: P.Parser<T>,
	stack: Array<S.Expression>
): P.Parser<S.Expression> {
	return P.lazy(() =>
...
function otherExpression<T>(
	end: P.Parser<T>,
	stack: Array<S.Expression>
): P.Parser<S.Expression> {
	return nodeMap(S.OtherExpression, withAny(end, false)).chain(
		tryOperator(end, stack)
	);
}
...
function withOperator<T>(
	...
	case 3:
		...
		return P.succeed(
			S.Ternary(combineMark(cond.mark, right.mark), cond, left, right)
		);
	default:
		throw new SoyParseError(
			`Error parsing an operator of length ${stack.length}.`
		);
	}
}

function ternaryLeft<T>(
	end: P.Parser<T>,
	stack: Array<S.Expression>
): P.Parser<S.Expression> {
	return P.whitespace
...
function orAny<T>(parser: P.Parser<T>): P.Parser<T> {
	const newParser: P.Parser<T> = P.lazy(() => parser.or(P.any.then(newParser)));
...
function withAny<T>(parser: P.Parser<T>, consumeEnd = true): P.Parser<string> {
	const newParser: P.Parser<string> = P.lazy(() =>
		P.alt(
			...
			P.seqMap(P.any, newParser, (s, next) => s + next)
		)
	);
...
export default function parse(input: string): S.Program {
	...
	if (!result.status) {
		throw new SoyParseError(
			`Parsing failed at ${result.index.line}:${
				result.index.column

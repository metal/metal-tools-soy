...
describe('Compile Soy Pipeline', function() {
	...
	it('should set the "params" variable for each template, with a list of its param names', function(
		...
		stream.on('data', function(file) {
			...
			assert.notStrictEqual(
				-1,
				contents.indexOf('exports.render.params = [];'),
			);
			assert.notStrictEqual(
				-1,
				contents.indexOf(
	...
	it('should add lines to generated soy js file that import some metal ES6 modules', function(
		...
		stream.on('data', function(file) {
			...
			assert.notStrictEqual(
				-1,
				contents.indexOf(`import Soy from 'metal-soy';`),
			);
			expect(contents).toMatchSnapshot();
			done();
		});
	...
	it('should export the templates', function(done) {
		...
		stream.on('data', function(file) {
			...
			assert.notStrictEqual(
				-1,
				contents.indexOf('export default templates;'),
			);
			expect(contents).toMatchSnapshot();
			done();
		});
	...
	it('should automatically generate and export component class using SoyRenderer', function(
		...
		stream.on('data', function(file) {
			...
			assert.notStrictEqual(
				-1,
				contents.indexOf('class Simple extends Component'),
			);
			assert.notStrictEqual(
				-1,
			...
			assert.notStrictEqual(
				-1,
				contents.indexOf('export { Simple, templates };'),
			);
			expect(contents).toMatchSnapshot();
			done();
		});

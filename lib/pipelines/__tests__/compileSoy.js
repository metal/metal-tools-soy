...
describe('Compile Soy Pipeline', function() {
	...
	it('should not add the source map path to the end of the generated file when skipMetalGeneration is true', function(
		done
	) {
		const stream = vfs.src('test/fixtures/soy/simple.soy').pipe(
			compileSoy({
		...
		stream.on('end', function() {
			...
			expect(RegexSourceMapping.test(contents)).toBeFalsy();

...
describe('Compile Soy Pipeline', function() {
	...
	it('should replace goog.require calls to other namespaced templates with Soy.getTemplate calls', function(done) {
		const stream = vfs
			.src([
	...
	it('should replace external messages from goog.getMsg calls if an externalMsgFormat has been specified and replace the params placeholders', function(done) {
		const externalMsgFormat = `I18n.translate('$2')`;

		const stream = vfs.src('test/fixtures/soy/messagesParams.soy').pipe(
			compileSoy({
				externalMsgFormat: externalMsgFormat
			})
		);
		...
		stream.on('data', function(file) {
			...
			expect(
				contents.indexOf(
					`I18n.translate('you-are-about-to-report-a-violation-of-our-x.-all-reports-are-strictly-x')`
				)
			).not.toBe(-1);

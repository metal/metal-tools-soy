...
const PATH_TO_JAR = path.resolve(
  __dirname,
  '../../jar/soy-2017-08-07-SoyToIncrementalDomSrcCompiler.jar'
);
...
module.exports = function(options) {
  ...
  return combiner(
    ...
    gulpif(
      ...
      replace(EXTERNAL_MSG_REGEX, 'var $1 = ' + options.externalMsgFormat + ';')
    ),
    through.obj()
  );
};
...
function extractTemplateData(soyData, template) {
  if (template.type !== 'DelTemplate') {
    ...
    let templateName = template.id.namespace ? template.id.namespace + '.' : '';
...
function extractParams() {
  return through.obj(function(file, encoding, callback) {
    try {
      ...
      e.message = 'Failed to parse soy file: ' + file.path + '\n' + e.message;

	...
	return through.obj(function(file, encoding, callback) {
		try {
			...
			parsed.body.forEach(function(template) {
				...
				var extractParam = function(param) {
					addTemplateParam(soyJsPath, templateName, param.name);
					templateData[soyJsPath][templateName].types[param.name] = param.paramType;
				}

				// Parse inner params
				template.params.forEach(extractParam);

				// Parse doc params (deprecated)
				if (template.doc && template.doc.params) {
					template.doc.params.forEach(extractParam);
				}
			});
